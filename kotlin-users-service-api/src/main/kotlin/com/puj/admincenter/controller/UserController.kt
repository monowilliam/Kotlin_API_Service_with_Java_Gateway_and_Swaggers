...
class UserController(private val userService: UserService) {
    ...
    // ACTUALIZAR CONTRASEÑA DADO EL USERNAME Y PASSWORD
    @PutMapping(
        consumes = ["application/json"],
        produces = ["application/json"]
    )
    fun update(@RequestBody @Valid UpdateUserPassword: UpdateUserPassword,
                @RequestHeader(value="authorization", required=true) authorization: String): ResponseEntity<*>
        = userService.update(UpdateUserPassword)
}